...
def trainModelsAndSave(df, update_about, gender="M"):
    ...
    if update_about:
        updateAbout(ind, dep, features, X_train, y_train)
...
def runModels(X_train, X_test, y_train, y_test, gender):
    ...
    print(
        f"Torvik Forest Model Training took: {(datetime.now() - start).total_seconds()}"
    )
    ...
    print(
        f"Torvik Decision Tree Model Training took: {(datetime.now() - start).total_seconds()}"
    )
...
def aboutForest(features, X_train, y_train):
    ...
    fig, ax = plt.subplots()
    ax.barh(
        features.columns[forest_sort],
        init_forest.feature_importances_[forest_sort],
        color=["blue"],
    )
