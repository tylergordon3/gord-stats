...
def save_ensemble_model(ensemble_package, version):
    ...
    filename = f"womens_{version}.pkl"
...
def train_random_forest(X, y):
    ...
    grid = GridSearchCV(
        RandomForestClassifier(random_state=13, class_weight="balanced"),
        param_grid,
        scoring="roc_auc",
        cv=StratifiedKFold(5, shuffle=True, random_state=13),
        n_jobs=-1,
...
def train_stacked_ensemble(X, y):
    ...
    rf_oof = generate_oof_predictions(train_random_forest, X_train, y_train)
    ...
    # 2. Train Meta Model
    # ---------------------------
    meta_X_train = np.column_stack([log_oof, rf_oof, ada_oof, gb_oof, hgb_oof])
    ...
    rf_full = train_random_forest(X_train, y_train)
    ...
    rf_test = rf_full["model"].predict_proba(X_test[rf_full["features"]])[:, 1]
    ...
    meta_X_test = np.column_stack([log_test, rf_test, ada_test, gb_test, hgb_test])
    ...
    save_ensemble_model(ensemble_package, version="2-20-2026")
