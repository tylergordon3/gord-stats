...
def getMasterTeams():
    ...
    """
    return pd.read_json(paths.MASTER_DICT)
...
def saveMasterTeams(df):
    ...
    """
    paths.MASTER_DICT.parent.mkdir(parents=True, exist_ok=True)
...
def get_image_name(team):
    ...
    for file_path in img_dir.glob("*"):
        if file_path.is_file():
            ...
            if file_stem in names:
                return file_path.name  # just return filename
